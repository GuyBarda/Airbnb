        ...
        <section :class="{ 'btn-selected': isSelect !== ' ' }" class="search-input">
            <div @click.prevent="openZone('Where')" :class="{ selected: isSelect === 'Where' }"
                ...
                <input class="dest-input" :class="{ set: filterBy.destination }" type="text"
                    v-model="filterBy.destination" placeholder="Search destinations" />
                <destination-modal @setDestination="setDestination" :class="{ open: isSelect === 'Where' }" />
            </div>
            <span></span>
            <section class="date">
                <div @click.prevent="openZone('In')" :class="{ selected: isSelect === 'In' }" class="search-zone align">
                    <label>Check in</label>
                    <input :class="{ set: formattedStartDate }" :value="formattedStartDate" class="date-input"
                        type="text" placeholder="Add dates" disabled />
                </div>
                <el-date-picker @change="setDates" ref="datePicker" popper-class="datePicker" v-model="dates"
                    type="daterange" start-placeholder="Start date" end-placeholder="End date" value-format="x" />
                <span></span>

                <div @click.prevent="openZone('Out')" :class="{ selected: isSelect === 'Out' }"
            ...
            <div @click.prevent="openZone('Who')" :class="{ selected: isSelect === 'Who' }"
