          ...
          <section class="below-imgs">
            ...
            >
              ...
              <select name="stay-type">
                <option
                  :selected="(house.type[0] = 'entire place')"
                  value="entire place"
                >
                  Entire place
                </option>
                <option
                  :selected="(house.type[0] = 'private room')"
                  value="private room"
                >
                  Private room
                </option>
                <option
                  :selected="(house.type[0] = 'shared room')"
                  value="shared room"
                >
                  Shared room
                </option>
              ...
              <div class="house-amenities">
                <div v-for="a in allAmenities" style="display: flex; gap: 13px">
                  <input
                    ...
                    type="checkbox"
                    :name="a"
                    :checked="house.amenities.includes(a)"
                  />
                  <img
...
export default {
  ...
  async created() {
    ...
    console.log("created", this.house.amenities);
  },
  computed: {
    rate() {
      ...
      return `${sum} (${this.house.reviews.length} ${
        this.house.reviews.length === 1 ? "Review" : "Reviews"
      })`;
    },
  },
  methods: {
